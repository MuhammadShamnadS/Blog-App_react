...
function TagList() {
  ...
  const fetchTags = async (page = 1) => {
    ...
    setError("");
    try {
      const res = await authService.fetchTags(id, page);
      setTags(res.data);
  ...
  const handleAddTag = async () => {
    ...
    try {
      ...
      if (err?.errors?.name) {
        setTagFieldError(err.errors.name[0]);
      } else if (err.response?.error) {
        setTagFieldError(err.error);
      } else {
  ...
  return (
    <Container maxWidth="lg">
      ...
      >
        <Button
          onClick={() => navigate("/dashboard/admin/category")}
          sx={{
      ...
      >
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleEditTag}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
