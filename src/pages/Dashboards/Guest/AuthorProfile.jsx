...
function AuthorProfile() {
  ...
  const toggleSubcribe = async (authorId) => {
    try {
      const res = await authService.subscribeAuthors(authorId);
      setAuthor((prev) => ({
        ...prev,
        is_subscribed: res.data.action === "subscribed",
      }));

      setSnackbar({
        open: true,
        message: res.data.action === "subscribed" ? "Subscribed successfully" : "Unsubscribed successfully",
        severity: "success",
      });
    } catch (err) {
            ...
            {author.is_followed && (
              <Button
                ...
                onClick={async () => {
                  ...
                  if (success) {
                    await toggleSubcribe(author.id);
                  }
                }}
              ...
              >
                {author.is_subscribed ? (
                  <>
                    <PersonRemoveIcon fontSize="small" /> Unsubscribe
                  </>
                ) : (
                  <>
                    <PersonAddIcon fontSize="small" /> Subscribe
                  </>
                )}
              </Button>
            )}
